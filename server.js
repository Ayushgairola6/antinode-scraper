...
// import express from "express";
// import { chromium } from "playwright-core"; // lightweight, no browser download
...
// // Health check
// // ------------------------------------------------------------------
// app.get("/", (req, res) => res.send("Antinode Scrape API is live 🚀 (Lightpanda)"));
...
// // ------------------------------------------------------------------
// // Scrape a single URL (using Lightpanda via CDP)
// // ------------------------------------------------------------------
// async function scrapeUrl(url, queryEmbedding, turndown) {
//     const browser = await getBrowser();
//     let page = null;
...
// const PORT = process.env.PORT || 7860;
// app.listen(PORT, "0.0.0.0", async () => {
//     await getBrowser(); // pre‑warm Lightpanda
//     console.log(`🚀 Lightpanda scraper running on port ${PORT}`);
// });
...
// Health check
// ------------------------------------------------------------------
app.get("/", (req, res) => res.send("Antinode Scrape API is live 🚀 (Lightpanda fetch)"));
...
async function processMarkdown({ markdown, url, hostname, queryEmbedding }) {
    ...
    // Try to extract a title (first line or fallback)
    let title = "";
...
async function scrapeUrl(url, queryEmbedding) {
    // Path to the Lightpanda binary (downloaded during postinstall)
...
// ------------------------------------------------------------------
// Main endpoint (unchanged except using new scrapeUrl)
// ------------------------------------------------------------------
app.post("/api/search", async (req, res) => {
    const { source, prompt, user_id, message_id, webhook_url } = req.body;
